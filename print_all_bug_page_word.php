...
# along with MantisBT.  If not, see <http://www.gnu.org/licenses/>.
    ...
    // Определяем локальную ф-цию для вывода комментариев  иначе конфликт имен
    function f_my_show_comments($in_bug_id)
    {
        $in_currbug_id_for_print = $in_bug_id;
...
//--MY_START_Merkelov полностью переделал код по образу вывода одного инцидента, до этого тут была полная хрень не учитывающая настройки видимости столбцов
	...
	for( $j=0; $j < $t_row_count; $j++ )
    {
		...
		if ( in_array( $t_id, $f_bug_arr ) || !$f_show_flag )
        {
            ...
            // пробел между инцидентами
            echo '<br />';

            //-----
            // Вывод текущего инцидента
...
/* Не выводим заголовок для каждого инцидента
                echo '<tr>';
