...
public function get_page($q='')
{
    ...
    $query = $this->query_paging($this->q);
    $r = mysql_query($query) or die('query='.$query.'; Err='.mysql_error());
    $r2 = mysql_query('SELECT FOUND_ROWS()');
    $this->total_rows = array_pop(mysql_fetch_row($r2));
    ...
    if ($this->cur_page > $this->total_pages)
    {
        ...
        if ( $this->total_pages > 0 ) $r = mysql_query( $this->query_paging($this->q) );
    }
